...
По умолчанию ставится **`ru_RU-irina-medium`** (женский, разборчивый,
~15 МБ). Другие варианты для русского:
...
# Другой женский
huggingface-cli download rhasspy/piper-voices \
  --include 'ru/ru_RU/anna/medium/*' --local-dir models/
```

После скачивания укажите путь через переменную:

```bash
export QUEST_TTS_VOICE=$(pwd)/models/ru_RU-dmitri-medium.onnx
.venv/bin/python3 -m daemon.quest_tts_daemon watch
```

Или отредактируйте `daemon/config.py` (`PIPER_VOICE`).

### Fallback: espeak-ng

Если piper по какой-то причине недоступен, демон переключится на
`espeak-ng` (качество ниже, зато работает без нейросетей):

```bash
sudo apt install espeak-ng
python3 -m daemon.quest_tts_daemon --backend espeak watch
```
